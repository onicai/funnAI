...
<div class="agent-card bg-agent-surface! {compact ? 'p-4' : 'p-5 sm:p-6'}">
  ...
  {#if loading}
    <div class="relative z-1 space-y-2">
      {#each Array(5) as _}
        <div class="animate-pulse grid grid-cols-12 gap-4 items-center p-3 rounded-xl bg-white/3 border border-white/4">
          <div class="col-span-1 flex justify-center">
            <div class="w-8 h-8 bg-white/8 rounded-full"></div>
          </div>
          <div class="col-span-4 space-y-2">
            <div class="h-3.5 bg-white/8 rounded-sm w-3/4"></div>
            <div class="h-3 bg-white/5 rounded-sm w-1/2"></div>
          </div>
          <div class="col-span-2"><div class="h-3.5 bg-white/8 rounded-sm w-16 ml-auto"></div></div>
          <div class="col-span-2"><div class="h-6 bg-white/8 rounded-full w-20"></div></div>
          <div class="col-span-2"><div class="h-3.5 bg-white/8 rounded-sm w-14 ml-auto"></div></div>
          <div class="col-span-1"><div class="h-3 bg-white/5 rounded-sm w-8 ml-auto"></div></div>
        </div>
      {/each}
    </div>
  {:else if !hasValidPrincipal}
    <div class="relative z-1 text-center py-10 rounded-xl border border-white/6 bg-white/3">
  ...
  {:else}
    ...
    <div class="relative z-1 hidden md:grid grid-cols-12 gap-4 px-3 py-2 mb-2 text-[11px] uppercase tracking-[0.14em] text-gray-500 font-medium border-b border-white/6">
    ...
    <div class="relative z-1 space-y-1 max-h-96 overflow-y-auto">
      {#each displayTransactions as tx (tx.id)}
        ...
        <div class="rounded-xl border border-transparent hover:border-white/8 hover:bg-white/3 transition-all duration-200 group">
          <div
          ...
          >
            <div class="flex items-start justify-between mb-3 gap-3">
              <div class="flex items-center gap-3 min-w-0">
                <div class="w-10 h-10 rounded-full {style.bgColor} flex items-center justify-center shrink-0">
          ...
          {#if expandedTransaction === tx.id}
            <div class="px-3 pb-3 border-t border-white/6 bg-white/2">
              <div class="pt-3 space-y-3">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <span class="text-[11px] font-medium text-gray-500 uppercase tracking-[0.14em]">Transaction ID</span>
                  <div class="flex items-center gap-2 min-w-0">
                    <code class="text-xs font-mono text-gray-300 bg-white/4 border border-white/6 px-2 py-1 rounded-lg break-all">{tx.id}</code>
                    <button type="button" on:click|stopPropagation={() => copyToClipboard(tx.id)} class="p-1.5 rounded-lg hover:bg-white/6 text-gray-500 hover:text-gray-300 transition-colors shrink-0">
                ...
                {#if tx.from}
                  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    ...
                    <code class="text-xs font-mono text-gray-300 bg-white/4 border border-white/6 px-2 py-1 rounded-lg break-all">{tx.from}</code>
                  </div>
                {/if}
                {#if tx.to}
                  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <span class="text-[11px] font-medium text-gray-500 uppercase tracking-[0.14em]">To</span>
                    <code class="text-xs font-mono text-gray-300 bg-white/4 border border-white/6 px-2 py-1 rounded-lg break-all">{tx.to}</code>
                  </div>
                {/if}
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border border-agent-purple/30 bg-agent-purple/15 text-[#c4b5fd] w-fit">
                    {tx.type.toUpperCase()}
                  </span>
                </div>
              </div>
    ...
    {#if showPagination && transactions.length > 0 && totalPages > 1}
      <div class="relative z-1 mt-5 pt-4 border-t border-white/6">
          ...
          {#each getVisiblePages(false) as page}
            ...
            {:else}
              <button
                ...
                disabled={loading}
                class="h-9! min-w-9! px-3! rounded-full text-[13px] font-medium transition-all duration-200 disabled:opacity-50 {page === currentPage
                  ? 'bg-agent-purple text-white shadow-xs'
                  : 'border border-white/10 bg-white/4 text-gray-300 hover:border-agent-purple/40 hover:text-white'}"
              >
                {page}
              </button>
            {/if}
          ...
          {#each getVisiblePages(true) as page}
            ...
            {:else}
              <button
                ...
                on:click={() => goToPage(typeof page === 'number' ? page : parseInt(page.toString()))}
                disabled={loading}
                class="h-9! min-w-9! px-3! rounded-full text-[13px] font-medium {page === currentPage
