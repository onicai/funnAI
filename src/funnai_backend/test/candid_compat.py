...
Adapted from ``llama_cpp_canister/test/candid_compat.py`` (v0.15.0). The
``strip_token_accounting`` helper is intentionally not carried over: it strips
fields that only exist on the llama_cpp canister's run_update/run_query records.
"""
