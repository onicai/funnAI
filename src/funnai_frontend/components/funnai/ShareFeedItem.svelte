...
<div bind:this={containerElement} class="share-menu-container relative">
  <button
    ...
    class="relative p-1 text-gray-500 hover:text-gray-300 transition-colors duration-200 rounded-md hover:bg-white/[0.05] {showShareMenu ? 'pointer-events-none' : ''}"
  ...
  {#if showShareMenu}
    ...
    <div 
      class="absolute right-0 top-8 bg-agent-elevated border border-white/10 rounded-xl shadow-xl py-2 min-w-[180px]" 
      on:click|stopPropagation 
      on:keydown|stopPropagation 
      style="z-index: 999999;"
    >
      <div class="px-3 py-1 text-xs font-medium text-gray-500 border-b border-white/10 mb-1">
        Share on
      </div>
      
      <button
        on:click={() => handleShare('twitter')}
        class="w-full px-3 py-2 text-sm text-left text-gray-300 hover:bg-white/[0.05] hover:text-white transition-colors flex items-center gap-2"
      >
