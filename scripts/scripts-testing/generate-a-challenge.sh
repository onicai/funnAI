...
CHALLENGER_LLM_0_BALANCE_0_=$(dfx canister --network $NETWORK_TYPE status $CHALLENGER_LLM_0 2>&1 | grep "Balance:"| awk '{print $2}')
...
if [ "$GENERATED_BY_LLM_ID" = "$CHALLENGER_LLM_0" ]; then
    LLM_INDEX=0
    echo "generatedByLlmId: $GENERATED_BY_LLM_ID is LLM 0: $CHALLENGER_LLM_0"
    LLM_CYCLES_CHANGE_1=$(echo "$LLM_BALANCE_1 - $CHALLENGER_LLM_0_BALANCE_0" | bc)
else
    LLM_INDEX=1
    echo "generatedByLlmId: $GENERATED_BY_LLM_ID is LLM 1: $CHALLENGER_LLM_1"
