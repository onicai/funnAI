...
def get_cycles_per_icp_from_cmc() -> int:
    """
    ...
    if xdr_permyriad_per_icp is None:
        raise ValueError("Error: Could not retrieve xdr_permyriad_per_icp from response")
    
    xdr_permyriad_per_icp = int(xdr_permyriad_per_icp)
    cycles_per_icp = (xdr_permyriad_per_icp * 1_000_000_000_000) // 10_000

    return cycles_per_icp

def get_usd_per_computed_xdr_from_cmc():
        """        
            Returns the USD value of 1 Computed XDR based on current ICP and cycles rates.
            This is the USD value of 1 Trillion cycles as computed by the CMC canister.
        """
        coinbase_usd_per_icp = get_icp_to_usd_rate_from_coinbase()
        cmc_cycles_per_icp = get_cycles_per_icp_from_cmc()
        cmc_usd_per_computed_xdr = coinbase_usd_per_icp / (cmc_cycles_per_icp / 1_000_000_000_000)
        return cmc_usd_per_computed_xdr

def get_cycle_burn_rate_from_ic_api():
    try:
        url = "https://ic-api.internetcomputer.org/api/v3/metrics/cycle-burn-rate"
        response = requests.get(url)
        data = response.json()
        return float(data["cycle_burn_rate"][0][1]) # cycles/second
    except Exception as e:
        raise RuntimeError(f"Failed to retrieve cycle burn rate: {e}")

def get_xdr_to_usd_rate_from_coinbase():
    try:
        url = "https://api.coinbase.com/v2/exchange-rates?currency=XDR"
        response = requests.get(url)
        data = response.json()
        return float(data["data"]["rates"]["USD"])
    except Exception as e:
        raise RuntimeError(f"Failed to retrieve XDR to USD rate: {e}")

def get_icp_to_usd_rate_from_coinbase():
    try:
        ...
        raise RuntimeError(f"Failed to retrieve ICP to USD rate: {e}")
    
def get_usd_per_trillion_cycles():
    usd_per_xdr = get_xdr_to_usd_rate_from_coinbase()
    return usd_per_xdr

def icp_xdr_summary():
    """
    Summarizes the current prices of ICP, XDR, and cycles.
    """
    coinbase_usd_per_xdr = get_xdr_to_usd_rate_from_coinbase()
    coinbase_usd_per_icp = get_icp_to_usd_rate_from_coinbase()
    cmc_cycles_per_icp = get_cycles_per_icp_from_cmc()
    ...
    print(f"💰 Coinbase -> 1 XDR ≈ ${coinbase_usd_per_xdr:.4f} USD")
    print(f"💰 Coinbase -> 1 ICP ≈ ${coinbase_usd_per_icp:.4f} USD")
    print(f"💰 CMC      -> Tcycles per ICP: {cmc_cycles_per_icp / 1_000_000_000_000:.4f} Tcycles/ICP")
    print(f"💰 CMC      -> 1 Computed XDR ≈ ${cmc_usd_per_computed_xdr:.4f} USD")
    print(f"💰 IC API   -> Cycle burn rate: {ic_api_cycle_burn_rate / 1_000_000_000_000:.4f} Tcycles/second ({ic_api_tcycle_burn_rate_per_day:.2f} Tcycles/day)")
