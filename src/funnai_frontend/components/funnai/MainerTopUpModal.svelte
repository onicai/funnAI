<script lang="ts">
  ...
  function calculateCycles() {
    if (!conversionRate || !amount || isNaN(Number(amount)) || Number(amount) <= 0) {
      cyclesAmount = "0";
      return;
    }
    
    try {
      const E8S_PER_ICP = new BigNumber("100000000"); // 10^8 e8s per ICP
      
      // Convert amount to e8s format
      const [integral, fractional = ""] = amount.split(".");
      const e8sAmount = new BigNumber(integral).times(E8S_PER_ICP)
        .plus(new BigNumber(fractional.padEnd(8, '0').substring(0, 8)));
  ...
  async function handleSubmit() {
    if (isValidating || !hasEnoughBalance) return;
    isValidating = true;
    errorMessage = "";
