...
def cleanup_llm_promptcache(gamestate_canister_id, judge_canister_id, share_service_canister_id, canister_name, canister_id, network):
    ...
    try:
        # -------------------------------------------------------------
        # We can delete the prompt cache files of all archived & closed challenges
        # -------------------------------------------------------------
        ...
        if network != "prd":
            # Ok for testing on demo network, where we do not have any archived challenges
            ...
            print("We only close prompt caches for archived challenges on production network.")

        # get promptIds for archived challenges
        print(f"Cleaning up prompt cache for archived challenges on network {network}...")
        result = subprocess.check_output(
        ...
        if llm_type == "judge" or llm_type == "all":
            ...
            for judgePromptId in judgePromptIds:
        ...
        if llm_type == "share_service" or llm_type == "all":
            ...
            for mainerPromptId in mainerPromptIds:
...
def main(network, llm_type):
    ...
    for name, canister_id in CANISTERS.items():
        ...
        if gamestate_name and judge_name and share_service_name:
            ...
            print(f"Found canisters: GAMESTATE={gamestate_name}, JUDGE={judge_name}, SHARE_SERVICE={share_service_name}")   
            break
    ...
    for name, canister_id in CANISTERS.items():
        if "LLM" in name.upper() and not "CHALLENGER" in name.upper() and (llm_type == 'all' or llm_type.lower() in name.lower()):
            ...
            cleanup_llm_promptcache(gamestate_canister_id, judge_canister_id, share_service_canister_id, name, canister_id, network)
