...
def analyze_balance_history(balance_history: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Analyze balance history to extract whitelist-relevant metrics.
    """
    if not balance_history:
        return {"error": "No balance history available"}
    
    # Get balance values over time
    balances = [point['balance_icp'] for point in balance_history if 'balance_icp' in point]
    
    if not balances:
        return {"error": "No balance data in history"}
    ...
    analysis = {
        ...
        "total_transactions": len([p for p in balance_history if p.get('event_type') == 'transaction']),
        "incoming_transactions": len([p for p in balance_history if p.get('transaction_type') == 'incoming_transfer']),
        "outgoing_transactions": len([p for p in balance_history if p.get('transaction_type') == 'outgoing_transfer']),
        "total_received": sum([p.get('amount_icp', 0) for p in balance_history if p.get('transaction_type') == 'incoming_transfer']),
        "total_sent": sum([p.get('amount_icp', 0) for p in balance_history if p.get('transaction_type') == 'outgoing_transfer']),
        "first_activity": balance_history[0]['timestamp'] if balance_history else None,
        "last_activity": balance_history[-1]['timestamp'] if balance_history else None
    }
    
    # Determine whitelist eligibility
    analysis['whitelist_score'] = calculate_whitelist_score(analysis)
    analysis['whitelist_tier'] = determine_whitelist_tier(analysis)
    
    return analysis


def calculate_whitelist_score(analysis: Dict[str, Any]) -> float:
    """
    Calculate a whitelist score based on ICP activity.
    Higher score = better whitelist candidate.
    """
    score = 0.0
    
    # Current balance scoring (0-40 points)
    ...
    # Historical max balance scoring (0-25 points)
    ...
    if max_balance >= 100:
        ...
        score += 15
    elif max_balance >= 0.1:
        score += 10
    
    # Transaction activity scoring (0-20 points)
    ...
    # Total volume scoring (0-15 points)
    total_received = analysis.get('total_received', 0)
    if total_received >= 1000:
        score += 15
    elif total_received >= 100:
        score += 12
    elif total_received >= 10:
        score += 8
    elif total_received >= 1:
        score += 5
    
    return score


def determine_whitelist_tier(analysis: Dict[str, Any]) -> str:
    """
    Determine whitelist tier based on analysis.
    """
    ...
    if score >= 80:
        ...
        return "tier_2_medium_priority"
    elif score >= 40:
        return "tier_3_low_priority"
    elif score >= 20:
        return "tier_4_very_low_priority"
    else:
        return "excluded"
