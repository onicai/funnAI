...
>
  <div class="space-y-4">
    ...
    {:else}
      <!-- Header Info -->
      <div class="rounded-xl border border-agent-purple/25 bg-agent-purple/10 p-4">
        <div class="flex items-start gap-3">
          <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-agent-purple/20 bg-agent-purple/15">
            <Flame size={20} class="text-agent-purple" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="agent-eyebrow mb-1">Performance</p>
            <h3 class="text-base font-semibold tracking-tight text-white">Unlock very high burn rate</h3>
      ...
      {#if funnaiToken}
        <div class="flex items-center gap-2 sm:gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/10">
          ...
          <div class="flex flex-col min-w-0 flex-1">
            <div class="text-white font-medium text-sm sm:text-base truncate">{funnaiToken.name}</div>
            <div class="text-xs sm:text-sm text-gray-400 truncate">Balance: {formatBalance(balance.toString(), funnaiToken.decimals)} {funnaiToken.symbol}</div>
          </div>
        </div>
      {/if}
      ...
      <div class="flex flex-col gap-3">
        ...
        <div>
          ...
          <input
            ...
            disabled
          />
          <div class="mt-1 text-xs text-gray-500">Target mAIner for Very High burn rate</div>
        </div>

        <!-- Burn Amount Display -->
        <div>
          <span class="block text-xs text-gray-400 mb-1.5">FUNNAI burn amount</span>
          <div class="relative">
            <input
              id="burn-amount-input"
              type="text"
              class="agent-input w-full pr-16 text-xs sm:text-sm"
              value={BURN_AMOUNT}
              disabled
            />
            <div class="absolute inset-y-0 right-0 flex items-center">
              <span class="pr-3 text-xs sm:text-sm text-gray-400 font-medium">FUNNAI</span>
            </div>
          </div>
        ...
        {#if !hasEnoughBalance && funnaiToken}
          <div class="p-2.5 rounded-xl border border-red-500/30 bg-red-500/10 text-red-300 text-xs sm:text-sm">
            Insufficient FUNNAI balance. You need {formatBalance(BURN_AMOUNT_BIGINT.toString(), funnaiToken.decimals)} FUNNAI but only have {formatBalance(balance.toString(), funnaiToken.decimals)} FUNNAI.
          </div>
        {/if}
        ...
        <button
          ...
          class="agent-btn-primary w-full h-11 disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={!canSubmit}
        >
          {#if isValidating}
...
<style>
  ...
  :global(.very-high-burn-rate-modal.modal-panel) {
