...
<div class="min-h-screen w-full bg-gray-900">
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold text-white mb-6">Access Codes & Lotteries</h1>
    
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <!-- Left Column -->
      <div class="lg:col-span-2">
        <div class="bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
          <h2 class="text-xl font-semibold text-white mb-4">Active Lotteries</h2>
          
          <!-- General Lottery -->
          {#if lotteries.length > 0}
            {#each lotteries.filter(l => l.type === "general") as lottery (lottery.id)}
              <div class="mb-6">
                <div class="bg-linear-to-r from-blue-50 to-indigo-50 from-blue-900/20 to-indigo-900/20 rounded-lg p-5 shadow-xs border border-blue-800">
                  <div class="flex justify-between items-start mb-3">
                    <div class="flex items-center">
                      <img src={lottery.image} alt={lottery.name} class="w-12 h-12 rounded-full object-cover mr-3 border-2 border-blue-700" />
                      <div>
                        <h3 class="text-xl font-bold text-blue-300">{lottery.name}</h3>
                        <p class="text-gray-400 mt-1">{lottery.description}</p>
                      </div>
                    </div>
                  </div>
                  
                  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <div class="bg-gray-900/30 p-3 rounded-md">
                      <div class="text-sm text-gray-400">Prize</div>
                      <div class="text-base font-medium text-gray-200">{lottery.prize}</div>
                    </div>
                    <div class="bg-gray-900/30 p-3 rounded-md">
                      <div class="text-sm text-gray-400">Next Draw</div>
                      <div class="text-base font-medium text-gray-200">{formatDate(lottery.nextDraw)}</div>
                    </div>
                    <div class="bg-gray-900/30 p-3 rounded-md">
                      <div class="text-sm text-gray-400">Participants</div>
                      <div class="text-base font-medium text-gray-200">{lottery.participants} / {lottery.slots} slots</div>
                    </div>
                  </div>
                  
                  {#if lottery.deposit > 0}
                    <div class="bg-yellow-900/20 border border-yellow-800 rounded-md p-3 mt-4">
                      <div class="font-medium text-yellow-300">Deposit Required</div>
                  ...
                  <div class="mt-4 flex items-center justify-between">
                    <div>
                      <span class="text-blue-400 font-semibold text-lg">{lottery.nextDraw.getHours() - new Date().getHours()}h {lottery.nextDraw.getMinutes() - new Date().getMinutes()}m</span>
                      <span class="text-gray-400"> remaining</span>
                    </div>
                    
                    {#if lottery.isRegistered}
                      <div class="flex items-center">
                        <div class="bg-green-900/30 px-3 py-1.5 rounded-md flex items-center mr-2">
                          <svg class="w-4 h-4 text-green-400 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                          </svg>
                          <span class="text-green-300 font-medium">Registered</span>
                        </div>
                      </div>
                    {:else}
                      <button 
          ...
          <!-- Whitelist Lotteries -->
          <h3 class="text-lg font-semibold text-white mt-8 mb-4">Whitelist Lotteries</h3>
            ...
            {#each lotteries.filter(l => l.type === "whitelist") as lottery (lottery.id)}
              <div class="bg-gray-700 border border-gray-600 rounded-lg p-4">
                <div class="flex justify-between items-start">
                  <div class="flex items-center">
                    <img src={lottery.image} alt={lottery.name} class="w-8 h-8 rounded-full object-cover mr-2 border border-purple-700" />
                    <h4 class="text-base font-semibold text-white">{lottery.name}</h4>
                  </div>
                  <span class="bg-purple-900/30 text-purple-300 text-xs font-medium px-2 py-0.5 rounded-sm">Whitelist</span>
                </div>
                
                <p class="text-sm text-gray-400 mt-2">{lottery.description}</p>
                
                <div class="mt-3 grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <span class="text-gray-400">Draw:</span>
                    <span class="text-white"> {formatDate(lottery.nextDraw)}</span>
                  </div>
                  <div>
                    <span class="text-gray-400">Prize:</span>
                    <span class="text-white"> {lottery.prize}</span>
                  </div>
                </div>
                ...
                <div class="mt-3 flex justify-between items-center">
                  ...
                  {#if lottery.isRegistered}
                    <div class="bg-green-900/30 px-2 py-1 rounded-sm text-xs text-green-300 font-medium">
                      Registered
                    </div>
                  {:else}
                    <div class="text-sm text-white bg-purple-700 px-3 py-1 rounded-md">
                      Need Access Code
                    </div>
                  {/if}
                </div>
      ...
      <div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
          ...
          <div class="border-t border-gray-700 pt-4">
            <h3 class="text-base font-semibold text-white mb-3">Access Code Benefits</h3>
        ...
        {#if results.length > 0}
          <div class="bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
            <h2 class="text-xl font-semibold text-white mb-4">Your Lottery Results</h2>
            
            {#each results as result (result.id)}
              <div class="bg-linear-to-r from-green-50 to-emerald-50 from-green-900/20 to-emerald-900/20 rounded-lg p-5 shadow-xs border border-green-800">
                <div class="flex justify-between items-start">
                  <div>
                    <div class="flex items-center">
                      <span class="text-xl font-bold text-green-300">Winner!</span>
                      <span class="ml-2 bg-green-900/30 text-green-300 text-xs font-medium px-2 py-0.5 rounded-sm">
                        {result.lotteryName}
                      </span>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div class="bg-gray-900/30 p-3 rounded-md">
                    <div class="text-sm text-gray-400">Draw Date</div>
                    <div class="text-base font-medium text-gray-200">{formatDate(result.drawDate)}</div>
                  </div>
                  <div class="bg-gray-900/30 p-3 rounded-md">
                    <div class="text-sm text-gray-400">Valid Until</div>
                    <div class="text-base font-medium text-gray-200">{formatDate(result.validUntil)}</div>
                  </div>
                </div>
                
                <div class="mt-4">
                  <div class="bg-yellow-900/20 border border-yellow-800 rounded-md p-3">
                    <div class="font-medium text-yellow-300">Time-Limited Offer</div>
                    <div class="text-sm text-gray-300 mt-1">
