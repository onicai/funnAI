...
<!-- Single module outline — internals use surface + spacing, not nested borders -->
<div class="agent-card font-sans">
  <div class="relative p-5 sm:p-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-5">
      ...
      <div class="flex flex-wrap items-center gap-2">
        {#if activeMainers > 0}
          <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[11px] font-medium text-emerald-400">
            <span class="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
            {activeMainers} mining
          </span>
        {/if}
        {#if inactiveMainers > 0}
          <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2.5 py-1 text-[11px] font-medium text-amber-400">
            <span class="h-1.5 w-1.5 rounded-full bg-amber-400"></span>
            {inactiveMainers} need attention
          </span>
        {:else}
          <span class="inline-flex items-center gap-1.5 rounded-full bg-white/[0.04] px-2.5 py-1 text-[11px] font-medium text-gray-400">
