    ...
    {:else}
      {#each feedItems as item (item.id)}
        <li
          role="article"
          class="relative pl-6
                 before:absolute before:z-10 before:left-0 before:top-2 before:h-2 before:w-2 before:-translate-x-1/2 before:rounded-full {getStatusColor(
            item.type,
          )} before:ring-2 before:ring-white animate-fadeIn"
          in:fly={{ y: 20, duration: 500 }}
        >
          <div class="flex flex-col flex-1 gap-2">
            <h4
              class="text-base font-medium flex justify-between items-center mr-6"
            >
              {item.mainerName}
            ...
            {#if item.type === "challenge"}
              <p class="text-slate-500">
                Received challenge: {item.content.challenge}
              </p>
            ...
            {:else if item.type === "winner"}
              <p class="text-slate-500">
                Achieved {item.content.placement} and earned {item.content
                  .reward}
              </p>
            {/if}
          </div>
