<script lang="ts">
  ...
  import Sidebar from "../components/SidebarChat.svelte";
  import Navigation from "../components/NavigationChat.svelte";
  ...
  onMount(() => {
    ...
    accordionInputs.forEach((input, index) => {
      input.addEventListener('change', () => {
        if (input.checked) {
          // Delay the scroll to allow other accordions to close
          setTimeout(() => {
          ...
          }, transitionDuration); // Adjust to match your CSS duration
        }
      });
    });
  });
      ...
      <div class="flex flex-col justify-between px-4 mx-auto max-w-screen-xl ">
        ...
        <div class="px-4">
          ...
          <div class="block p-6 bg-white border border-gray-200 rounded-lg shadow mb-4">
            <h5 class="mb-2 text-4xl font-bold tracking-tight text-gray-600">Let's build decentralized AI for education</h5>
