...
import { allowanceStore } from "../stores/allowanceStore";
...
export class IcrcService {
  ...
  private static async _batchGetBalances(
    ...
    const subaccount = storeState.accountId
      ? (Array.from(storeState.accountId) as number[])
      : undefined;
  ...
  public static async checkAndRequestIcrc2Allowances(
    token: FE.Token,
    payAmount: bigint,
    spender: string = backendCanisterId,
  ): Promise<bigint | null> {
  ...
  public static async checkIcrc2Allowance(
    ...
    try {
      const actor = await store.getActor(token.canister_id, canisterIDLs.icrc2, {
        anon: true,
