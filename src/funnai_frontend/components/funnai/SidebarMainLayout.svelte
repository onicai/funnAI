<script lang="ts">
  ...
  const currentPath = $derived(router.location);
...
<div class="sidebar-header font-sans flex flex-col h-full min-h-0 bg-transparent">
      ...
      {#each navItems as item}
        ...
        <li>
          ...
          >
            {#if active}
              <span
                class="nav-active-rail absolute top-2 bottom-2 left-0 w-[3px] rounded-r-full bg-agent-purple"
                aria-hidden="true"
              ></span>
            {/if}
            <svelte:component
  ...
  <div class="shrink-0 px-3 pt-3 pb-[max(1rem,env(safe-area-inset-bottom))] border-t border-white/6">
    <div class="rounded-2xl border border-white/8 bg-linear-to-b from-white/4 to-transparent p-3">
      <div class="px-1 mb-3">
      ...
      <a
        ...
        class="group flex items-center gap-3 rounded-xl border border-agent-purple/25 bg-agent-purple/12 px-3 py-2.5 transition-all duration-200 hover:border-agent-purple/45 hover:bg-agent-purple/18"
        aria-label="Open support community"
        onclick={closeSidebar}
      >
        <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/6 text-[#c4b5fd]">
      ...
      <div class="mt-2.5 grid grid-cols-2 gap-2">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          class="group flex items-center gap-2 rounded-xl border border-white/10 bg-white/3 px-2.5 py-2 text-gray-400 transition-colors hover:border-agent-purple/35 hover:bg-agent-purple/10 hover:text-gray-200"
