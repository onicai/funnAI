...
<div class="flex items-start justify-end gap-2.5 mt-4">
  <div class="flex flex-col w-full max-w-[20rem] leading-1.5 px-4 py-3 border-gray-200 bg-blue-600 rounded-xl rounded-tl-none">
    <p class="text-sm font-normal py-2 text-white">
      {@html marked(messageContent)}
    </p>
    <!-- Copy icon -->
    <span class="text-sm cursor-pointer font-normal text-gray-300 ml-auto " on:click={() => copyToClipboard(messageContent)}>
  ...
  <img class="w-8 h-8 rounded-full border-solid border-2 p-1 border-gray-200" src="/chat/devinci192.png" alt="User Logo">
</div> 