        ...
        <div class="relative z-1 p-5 sm:p-6">
          <div class="flex items-start justify-between gap-3 mb-5">
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                ...
                {#if $store.isAuthed && !showHoldingsSkeleton && listedCount > 0}
                  <span class="inline-flex items-center rounded-full border border-white/10 bg-white/4 px-2 py-0.5 text-[11px] font-medium text-gray-400">
            ...
            {#if $store.isAuthed}
              <button
                ...
                class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/4 text-gray-300 transition-all hover:border-agent-purple/40 hover:bg-agent-purple/10 hover:text-white disabled:opacity-40"
          ...
          {#if $store.isAuthed}
            ...
            {:else if loadingError && Object.keys(walletData.balances || {}).length === 0}
              <div class="rounded-xl border border-red-500/20 bg-red-500/8 px-4 py-8 text-center">
            ...
            {:else if walletData.tokens.length === 0}
              <div class="rounded-xl border border-white/6 bg-white/2 px-4 py-10 text-center">
                <p class="text-sm text-gray-400">No tokens available yet</p>
              </div>
            {:else}
              {#if loadingError && !isRefreshingHoldings}
                <div class="mb-4 rounded-xl border border-amber-500/20 bg-amber-500/8 px-4 py-3 text-sm text-amber-200">
            ...
            {/if}
          {:else}
            <div class="flex flex-col sm:flex-row sm:items-center gap-4 rounded-xl border border-white/[0.07] bg-white/2 p-4 sm:p-5">
