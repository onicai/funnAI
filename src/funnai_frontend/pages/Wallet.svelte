...
<div class="agent-page">
  <div class="agent-container">
    ...
    <div class="flex flex-col justify-center">
      <WalletStatus />
      <div class="agent-card p-6 mt-5">
        <h2 class="text-xl font-semibold tracking-tight text-white mb-4">Your Assets</h2>
        {#if $store.isAuthed}
          <!-- Content with consistent height to prevent flickering -->
          <div class="min-h-[340px] relative">
            {#if isDataLoading}
              <TokenListSkeleton rows={4} />
              <div class="absolute inset-0 bg-agent-bg/80 flex items-center justify-center rounded-xl">
                <LoadingIndicator text={"Loading wallet data..."} size={24} />
              </div>
            {:else if loadingError}
              <div class="flex flex-col items-center justify-center py-12">
                <div class="text-red-400 mb-4 text-center">{loadingError}</div>
                <button
        ...
        {:else}
          <div class="text-center py-10 rounded-xl border border-white/[0.06] bg-white/[0.03]">
            <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full border border-white/5 bg-white/[0.05]">
            ...
            <p class="mt-1 text-sm text-gray-400">Please connect your wallet to view your assets</p>
            <button on:click={connect} class="agent-btn-primary mt-5">
              Connect Wallet
            </button>
          </div>
        {/if}
