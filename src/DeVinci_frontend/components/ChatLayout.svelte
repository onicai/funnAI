  ...
  onMount(async () => {
    ...
    if (sidebarToggle && chatSidebar) {
      ...
      document.body.addEventListener('click', function(event) {
        ...
        if (!chatSidebar.contains(target) && !sidebarToggle.contains(target)) {
          chatSidebar.classList.add('-translate-x-full');
        }
      });

      chatSidebar.addEventListener('click', function(event) {
        event.stopPropagation();
      });
    }
...
<div class="App">
  ...
  {:else}
    <div class="flex flex-row h-screen">
      <aside id="chatSidebar" class="fixed z-50 bg-gray-200 w-72 min-w-72 h-full md:shadow transform -translate-x-full md:translate-x-0 transition-transform duration-150 ease-in">
      ...
      <main class="main flex flex-col flex-grow ml-0 md:ml-72 transition-all duration-150 ease-in">
        <header class="header bg-white shadow py-2 px-4">
          <div class="header-content flex items-center flex-row">
            <button id="chatSidebarToggle" data-drawer-target="chatSidebar" data-drawer-toggle="chatSidebar" aria-controls="chatSidebar" type="button" class="inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200">
